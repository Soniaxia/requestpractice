...
class RestClient():
    def __init__(self,api_root_url,username=None,password=None,token=None):
        ...
        if username and password:
            ...
            self.session.headers["Authorization"]="token {}".fomat(token)
    ...
    def options(self,url,**kwargs):
        return self.request(url,"opotions",**kwargs)
...
if __name__ =='__main__':
    r=RestClient("https://testtechapp.unifgroup.com")
    x=r.post("/user/1.0.0/login/accessToken",json={"account":"18408248103","password":"q123456"})
